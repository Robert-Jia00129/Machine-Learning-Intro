...
def occlusion(model, img_num=10, patch_size=4):
    ...
    true_prediction = y_test[num]
    y_pred = model.predict(img.reshape(-1,32,32,3))
    pred_wo_occlusion = np.zeros(10); pred_wo_occlusion[np.argmax(y_pred)] = 1

    # Get the loss of the model with the original image
    loss_nol = model.evaluate(img.reshape(-1,32,32,3), y_test[num].reshape(-1,10), verbose=0)[0]
    ...
    for top_left_x in range(0, img.shape[0], PATCH_SIZE):
        ...
        for top_left_y in range(0, img.shape[1], PATCH_SIZE):
            ...
            pred_with_occlusion = model.evaluate(patched_image.reshape(-1,32,32,3), 
                                                 pred_wo_occlusion.reshape(-1,10), verbose=0)
            
            loss_ol = pred_with_occlusion[0]

            # Get the loss_map of the plot by computing the difference in loss from the original version to the patched value
            loss_map[top_left_y:top_left_y + PATCH_SIZE, top_left_x:top_left_x + PATCH_SIZE] = np.abs(loss_nol - loss_ol)
    ...
    ax[0].imshow(img)
    ax[1].imshow(img.sum(axis=2), cmap=cmap)
    im = ax[1].imshow(loss_map, cmap=cmap, alpha=0.7)
